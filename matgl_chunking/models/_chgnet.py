...
class CHGNet(MatGLModel):
    ...
    def __init__(
        ...
        mem_log_path: str | None = None,
        mem_log_once: bool = True,
        **kwargs,
    ):
        """
        Args:
    ...
    def forward(
        ...
        if self.mem_log_path and (not self.mem_log_once or not self._mem_log_done):
            ...
            if use_edges and bond_graph is not None:
                items["bond_graph.ndata"] = dict(bond_graph.ndata)
                items["bond_graph.edata"] = dict(bond_graph.edata)
                items["angle_features"] = angle_features
            log_nbytes(self.mem_log_path, "CHGNet.forward", items)
        ...
        if self.readout_field == "atom_feat":
            ...
            if not use_edges:
                raise ValueError("readout_field='angle_feat' requires use_edges=True.")
